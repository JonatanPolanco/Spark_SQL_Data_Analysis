...
SELECT 
  ...
  CAST(FROM_UNIXTIME(VALUE.TIME) AS date) AS dates
...
AS (
  SELECT COUNT(*) AS count_broken_readings, dates
  FROM SILVER_TABLE
  WHERE heartrate < 0
  GROUP BY dates
  ORDER BY dates);
...
AS (
  ...
  FROM (
    SELECT *,
      LAG(heartrate) OVER (PARTITION BY device_id, dates ORDER BY device_id, dates) AS prev,
      LEAD(heartrate) OVER (PARTITION BY device_id, dates ORDER BY device_id, dates) AS next
    FROM SILVER_TABLE
    )
WHERE heartrate < 0
);
