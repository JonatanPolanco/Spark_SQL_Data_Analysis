...
SELECT 
  ...
  signal,
  temps,
  to_timestamp(timestamp, "yyyy/MM/dd HH:mm:ss") time_stamp
FROM ENERGY;
...
SELECT
  battery_level,
  device_id,
  FILTER (battery_level, level -> level < 0) need_service
FROM DCDevices
)
WHERE size(need_service) > 0
...
SELECT
  device_id,
  device_type,
  FILTER (co2_level, level -> level > 1400) highCO2,
  time_stamp
FROM DCDevices
)
